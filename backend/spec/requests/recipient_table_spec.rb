...
describe 'RECIPIENT API' do
  before(:each) do
    ...
    @user.save
    @recipient = Recipient.new(firstname: 'Jane', lastname: 'Doe', email: 'jane@email.com', user_id: @user.id,
                               tags: ['RSpec'])
    @recipient.save
  ...
  describe 'POST /recipients' do
    it 'creates a new recipient' do
      ...
      end.to change { Recipient.count }.from(1).to(2)
      expect(response).to have_http_status(:success)
    end
  ...
  describe 'UPDATE /recipients' do
    ...
    it 'is invalid if parameters to update are not correct' do
      patch "/api/v1/recipients/#{@recipient.id}", params: invalid_params,
                                                   headers: { 'Authorization' => "Bearer #{@token}" }
      expect(JSON.parse(response.body)['status']).to eq(400)
      expect(JSON.parse(response.body)['error']).to eq('Bad Request')
    end
  ...
  describe 'GET /recipients/export' do
    it 'returns the csv file' do
      ...
      expect(CSV.parse(response.body)[0]).to eq(%w[firstname lastname email tags])
